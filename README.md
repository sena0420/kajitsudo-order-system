...
- レスポンシブデザイン（xs / sm / md ブレークポイント対応）
- タッチフレンドリーなUI（最低44pxのタッチターゲット）
- ナビゲーションバーはモバイルでアイコンのみ表示に自動切替
- ダイアログはモバイルでフルスクリーン表示に自動切替
- 横スクロールが必要なテーブルにスワイプヒントを表示
...
### ⚠️ 不良報告機能（NEW！）
- 顧客が納品済み商品の品質不良を報告できる専用タブ
- 発注履歴（納品済みのみ）から対象商品を選択
- 不良数量（必須）・メモ（任意）・画像添付（任意・最大5枚）を入力して送信
- 画像プレビュー・個別削除に対応
- 送信済み報告の一覧表示（ステータス: 受付済み / 対応中 / 解決済み）
- Firebase Storage への画像アップロード（進捗バー付き）
- デモモードでは localStorage に保存
...
#### 管理者向け全顧客ビュー（NEW！）
- **発注タブ（管理者）**: 全顧客の商品一覧を顧客ごとのアコーディオンセクションで表示
  - 商品数チップで各顧客の商品数を一目確認
  - 発注回数・単価・規格・産地を閲覧（発注操作は不要なため読み取り専用）
- **履歴タブ（管理者）**: 全顧客の発注履歴を顧客ごとのアコーディオンセクションで表示
  - 未処理件数チップで優先対応が必要な顧客を即座に把握
  - 管理者による数量変更にも対応
...
- **Firebase Storage** - 不良報告の画像アップロード
...
│   │   ├── LoginPage.js               # ログイン画面
│   │   ├── DeliveryLocationSelector.js  # 納品先選択画面
│   │   ├── OrderPage.js               # 発注画面（バイヤー）
│   │   ├── OrderHistory.js            # 発注履歴画面（バイヤー）
│   │   ├── DefectReportPage.js        # 不良報告画面（NEW！）
│   │   ├── AdminPage.js               # マスタ管理画面（管理者）
│   │   ├── AdminOrderPage.js          # 商品一覧画面（管理者・NEW！）
│   │   ├── AdminOrderHistory.js       # 発注履歴画面（管理者・NEW！）
│   │   └── OrderManagementPage.js     # 受注管理画面（管理者）
│   ├── contexts/
...
- **[CONVERSATION_LOG2.md](./CONVERSATION_LOG2.md)** - 2025年10月13日〜2026年4月15日（現在）
